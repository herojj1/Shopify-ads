...
=============================================
Pure checkout automation engine with STRICT charge verification.

v2.3.0:
  - STRICT CHARGED verification: only marks CHARGED with real order proof
  - ProcessedReceipt/SuccessfulReceipt without order URL = APPROVED
  - Fixed actions JS regex for new Shopify CDN patterns
  - Added _verify_real_charge() helper
"""
...
# ═══════════════════════════════════════════════════════════════════
# STRICT CHARGE VERIFIER — THE KEY FIX
# ═══════════════════════════════════════════════════════════════════

def _verify_real_charge(poll_body: str) -> Tuple[bool, str]:
    """
    Return (is_real_order, receipt_url).

    A charge is REAL only when Shopify confirms a genuine order:
      • confirmationPage.url contains /orders/
      • OR orderStatusPageUrl contains /orders/
      • OR order.id / orderId matches gid://shopify/Order/<digits>
      • AND purchaseOrder exists (not null)
      • AND amount is non-zero

    Shopify's ProcessedReceipt alone is NOT proof — it can flip to
    FailedReceipt on the next poll.
    """
    try:
    ...
    # Check 3: confirmationPage.url with /orders/
    conf_url = ""
    ...
    # Check 4: orderStatusPageUrl with /orders/
    osp = receipt.get("orderStatusPageUrl") or ""
    if osp and "/orders/" in osp:
        return True, osp

    # Check 5: order GID fallback
...
def extract_actions_js_url(checkout_html: str, shop_url: str) -> str:
    """
    Extract the Shopify checkout actions JS URL.
    v2.3.0: matches all known CDN URL patterns.
    """
    patterns = [
...
_SHOPIFY_ERROR_MAP: Dict[str, str] = {
    ...
    "incorrect cvc": "CVV_INVALID", "incorrect_cvc": "CVV_INVALID", "security code": "CVV_INVALID",
...
def send_proposal(client: TLSClient, shop_url: str, checkout_url: str, checkout_token: str,
  ...
  "variables": {{
    ...
    "buyerIdentity": {{"customer": {{"presentmentCurrency": "USD", "countryCode": "US"}}, "phoneCountryCode": "US", "marketingConsent": [], "shopPayOptInPhone": {{"countryCode": "US"}}, "rememberMe": false}},
...
def send_proposal2(client: TLSClient, shop_url: str, checkout_url: str, checkout_token: str,
  ...
  "variables": {{
    ...
    "memberships": {{"memberships": []}},
    "payment": {{"totalAmount": {{"any": true}}, "paymentLines": [], "billingAddress": {{"streetAddress": {{"address1": "", "city": "", "countryCode": "US", "lastName": "", "phone": ""}}}}}},
    "buyerIdentity": {{"customer": {{"presentmentCurrency": "USD", "countryCode": "US"}}, "email": "{email}", "emailChanged": true, "phoneCountryCode": "US", "marketingConsent": [], "shopPayOptInPhone": {{"countryCode": "US"}}, "rememberMe": false}},
    "tip": {{"tipLines": []}}, "poNumber": null,
    "taxes": {{"proposedAllocations": null, "proposedTotalAmount": {{"any": true}}, "proposedTotalIncludedAmount": null, "proposedMixedStateTotalAmount": null, "proposedExemptions": []}},
    "note": {{"message": null, "customAttributes": []}},
    "localizationExtension": {{"fields": []}},
...
def send_proposal3(client: TLSClient, shop_url: str, checkout_url: str, checkout_token: str,
  ...
  "variables": {{
    ...
    "memberships": {{"memberships": []}},
    "payment": {{"totalAmount": {{"any": true}}, "paymentLines": [], "billingAddress": {{"streetAddress": {{
      "address1": "{addr.address1}", "address2": "{addr.address2}",
      "city": "{addr.city}", "countryCode": "{addr.country_code}",
      "postalCode": "{addr.postal_code}", "firstName": "{addr.first_name}",
      "lastName": "{addr.last_name}", "zoneCode": "{addr.zone_code}",
    ...
    "buyerIdentity": {{"customer": {{"presentmentCurrency": "USD", "countryCode": "US"}}, "email": "{email}", "emailChanged": false, "phoneCountryCode": "US", "marketingConsent": [], "shopPayOptInPhone": {{"countryCode": "US"}}, "rememberMe": false}},
    "tip": {{"tipLines": []}}, "poNumber": null,
    "taxes": {{"proposedAllocations": null, "proposedTotalAmount": {{"any": true}}, "proposedTotalIncludedAmount": null, "proposedMixedStateTotalAmount": null, "proposedExemptions": []}},
    "note": {{"message": null, "customAttributes": []}},
    "localizationExtension": {{"fields": []}},
...
def run_checkout_for_card(shop_url: str, card_entry: str, proxy_url: str = "", low: bool = True) -> CheckResult:
    """Enhanced version with STRICT charge verification."""
    currency = "USD"
    country = "US"
    ...
    try:
        ...
        # Step 6 — shipping country fallback
        try:
        ...
        try:
            ...
            if not receipt_id:
                ...
                if error_msg:
                    ...
                    result.status = CheckStatus.ERROR
                    result.error = Exception("Step 10: could not extract receiptId or error")
                    result.retryable = True
                return result
        ...
        for poll_num in range(1, 21):
            try:
                ...
                # ── STRICT CHARGED VERIFICATION ──
                if receipt_type in ["SuccessfulReceipt", "ProcessedReceipt"]:
                    real_order, real_url = _verify_real_charge(poll_body)

                    if real_order:
                        # ★ REAL CHARGE — all proof checks passed ★
                        result.status = CheckStatus.CHARGED
                        result.status_code = "ORDER_PLACED"
                        result.amount = total_amount
                        result.receipt_url = real_url or f"{shop_url.rstrip('/')}/orders"
                        logger.info("★ REAL CHARGE CONFIRMED — receipt=%s", result.receipt_url)
                        return result

                    # No real order proof — keep polling, cap at 15
                    if poll_num < 15:
                        logger.info("poll#%d — receipt without order proof, continuing...", poll_num)
                    else:
                        result.status = CheckStatus.APPROVED
                        result.status_code = "PAYMENT_PROCESSING"
                        result.amount = total_amount
                        result.receipt_url = ""
                        logger.info("poll#%d — no order proof after 15 polls, APPROVED", poll_num)
                        return result
                ...
                if receipt_type == "FailedReceipt":
                    ...
                    if "CAPTCHA" in error_code:
                        ...
                        if "InventoryReservationFailure" in poll_body:
                            ...
                            result.retryable = True
                            return result
                        _site_signals = ["fraud", "not supported", "brand", "suspected",
